...
def meaned(numpy_data):
    return np.mean(numpy_data, axis=2)

def stdied(numpy_data):
    return np.std(numpy_data, axis=2)
...
def mean_std(numpy_data):
    return np.concatenate((meaned(numpy_data), stdied(numpy_data)), axis=1)
...
def remove_sensors(numpy_data, sensors_to_remove):
    return np.delete(numpy_data, sensors_to_remove, axis=1)
...
def accelerations_velocities_abs(numpy_data):
    ...
    return np.array(list(map(abs, numpy_data)))
